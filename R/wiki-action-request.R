...
#' the name of the generator. You can `get_generator_of` with
#' `get_page_properties` to choose what properties of each page to return.
#'
#' **NB:** When setting additional parameters for a generator, ensure to prepend
#' a `g` to the parameter name. E.g. when generating members of a category using
#' `gcategorymembers`, you can set the limit for the request by setting the
#' `gcmlimit` parameter
#'
#' @param .req A httr2::request, e.g. generated by `wiki_action_request`
...
#'
#' resp <- wiki_action_request() %>%
#'   get_generator_of("gsearch", srsearch="seagull") %>%
#'   httr2::req_perform() %>%
#'   httr2::resp_body_json()
#'
#' resp$query$pages
get_generator_of <- function(.req, generator, ...) {
...
#'
#' `get_page_properties` can be used in combination with `get_generator_of`
...
#'
#' resp <- wiki_action_request() %>%
#'   get_generator_of("search", srsearch="seagull") %>%
