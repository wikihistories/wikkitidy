...
#'   get_list_of("categorymembers", cmsort = "timestamp",
#'     cmdir = "desc", cmtitle = "Category:Physics") %>%
#'   httr2::req_perform() %>%
#'   httr2::resp_body_json()
#'
#' resp$query$categorymembers
get_list_of <- function(.req, list, ...) {
...
#' the name of the generator. You can `generate_pages_by` with
#' `get_page_properties` to choose what properties of each page to return.
#'
#' When selecting a generator to use, or when setting additional
#' parameters, you must prepend a "g". For example, to generate pages
#' by the "categorymembers" list, you must pass "gcategorymembers" as the
#' generator name. Likewise, to set the limit on the number of category
#' members to return, you need to pass the parameter "gcmlimit"
#'
#' @param .req A httr2_request, e.g. generated by `wiki_action_request`
...
#'
#' resp <- wiki_action_request() %>%
#'   generate_pages_by("gsearch", gsrsearch="seagull") %>%
#'   httr2::req_perform() %>%
#'   httr2::resp_body_json()
#'
#' resp$query$pages
generate_pages_by <- function(.req, generator, ...) {
...
#'
#' `get_page_properties` can be used in combination with `generate_pages_by`
...
#' `get_page_properties` to collect information about them.
#'
#' @param .req A httr2_request, e.g. generated by `wiki_action_request`
...
#'
#' resp <- wiki_action_request() %>%
#'   generate_pages_by("search", srsearch="seagull") %>%
#'   get_page_properties("langlinks") %>%
#'   httr2::req_perform() %>%
#'   httr2::resp_body_json()
#'
#' resp$query$pages
get_page_properties <- function(.req, properties, ...) {
